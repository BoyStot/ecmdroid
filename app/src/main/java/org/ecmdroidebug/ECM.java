...
import org.ecmdroidebug.Constants.DataSource;
import org.ecmdroidebug.Constants.Variables;
import org.ecmdroidebug.EEPROM.Page;
import org.ecmdroidebug.Error.ErrorType;
import org.ecmdroidebug.PDU.Function;
...
public class ECM {
	...
	synchronized PDU sendPDU(PDU pdu) throws IOException {
		try {
			...
			Utils.postDebug(context,pdu.toString());
			if (out == null) {
			...
			// Wait for response
			PDU ret = receivePDU();
			Utils.postDebug(context,ret.getBytes());
			if (!ret.isResponse()) {
				Utils.postDebug(context,"No valid response from ECM (wrong Protocol?)");
				throw new IOException("No valid response from ECM (wrong Protocol?)");
			}
			if (!ret.isACK()) {
				Utils.postDebug(context,"Request not acknowledged by ECM (error code " + ret.getErrorIndicator() + ").");
	...
	public PDU receivePDU() throws IOException {
		try {
			...
			if (mReceiveBuffer[0] != PDU.SOH && mReceiveBuffer[4] != PDU.EOH && mReceiveBuffer[5] != PDU.SOT) {
				Utils.postDebug(context,mReceiveBuffer);
			...
			try {
				...
				Utils.postDebug(context,"ParseException");
				Utils.postDebug(context,mReceiveBuffer);
	...
	private int read(byte[] buffer, int offset, int len, int timeout) throws IOException {
		...
		if (r != len) {
			Utils.postDebug(context,Utils.hexdump(buffer, 0, r));
